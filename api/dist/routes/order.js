...
var validateMinMax = function validateMinMax(val) {
  return _config.simplex.minBaseCurrency <= val && _config.simplex.maxBaseCurrency >= val;
};
var validateAddress = function validateAddress(val) {
  for (var i = 0; i < _config.simplex.validDigital.length; i++) {
    try {
      if (_walletAddressValidator2.default.validate(val, _config.simplex.validDigital[i], 'prod')) {
        return true;
      }
    } catch (e) {
      continue;
    ...
    payment_details: {
      ...
      destination_wallet: {
        ...
        address: {
          ...
          message: 'destination address is required and must be a valid address'
        }
      }
    }
  }
