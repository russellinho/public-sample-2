...
const validateMinMax = val => {
  return simplex.minBaseCurrency <= val && simplex.maxBaseCurrency >= val
}
const validateAddress = val => {
  for (var i = 0; i < simplex.validDigital.length; i++) {
    try {
      if (wav.validate(val, simplex.validDigital[i], 'prod')) {
        return true
      }
    } catch (e) {
      continue
